...
const trainUntrainedItems = async (
  ...
  try {
    ...
    console.log(
      `Combined ${allItems.length} items for training in batches of 25`,
    );

    // Process all items in batches of 25 regardless of category
    while (allItems.length > 0) {
      // Get the next batch of up to 25 items
      const batchItems = allItems.splice(0, 25);
  ...
  const getTrainingStatusMessage = useCallback(() => {
    ...
    if (!steps || !steps.length) {
      ...
      if (remainingCount > 0) {
        ...
        return `Processing items in batches of 25... ${remainingCount} items remaining${categoryBreakdown}.`;
      }
